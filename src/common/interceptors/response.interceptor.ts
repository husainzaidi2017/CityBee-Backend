 ...
 *
 * Services return `{ data, pagination? }` from list endpoints; everything
    ...
    return next.handle().pipe(
      map((payload) => {
        if (payload && typeof payload === 'object' && 'data' in (payload as ListMeta)) {
          const { data, pagination, message } = payload as ListMeta & {
            data: unknown;
            message?: string;
          };
          return { success: true, data, message: message ?? 'Success', ...(pagination ? { pagination } : {}) };
        }
